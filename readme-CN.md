...
`headerSize` 不易设得过大，建议为 `10240` 到 `51200`。如果 `headerSize` 设置小于 1024，程序会自动将其调整为 1024。

`bufferSize` 定义的是读文件缓冲区的长度，以提高 IO 速度。如果 `bufferSize` 小于 `headerSize`，程序会自动将其调整为 `headerSize` 的值。

以我的一个备份为例，有 `216878` 个大于等于 10 字节的文件，主要为图片、视频、音乐、压缩文件、Word、PPT、程序源码、一些软件的安装包，共约 806GB。采用以下设置，以 3 种算法分别扫描。

- `bufferSize` 设为 `102400`，即 100KB。
- `headerSize` 分别设为 `10240`、`20480` 和 `40960`。
- 分别采用 `CRC32-IEEE`、`CRC64-ISO` 和 `MD5` 扫描文件。
- `needFullChecksum` 设置为 false，见后面说明。

得到不重复的 `headerChecksum` 数量，以及文件长度小于等于 `headerSize` 的文件数量如下：

| headerSize | CRC32 | CRC64 | MD5 | 小文件数量 | 小文件占比 |
| :---: | :---: |:---: |:---: | :---: | :---: |
| 10KB | 158237 | 158251 | 158251 | 105153 | 48.5% |
| 20KB | 158580 | 158594 | 158594 | 129644 | 59.8% |
| 40KB | 158826 | 158839 | 158839 | 144089 | 66.4% |

> 小文件占比是小文件数量除以文件总数 `216878`。这些小文件会自动得到 `fullChecksum`。

分析以上结果得到如下结论：

1. `headerSize` 越大，得到的 `headerChecksum` 数量越多，但增长幅度有限。
2. 使用 `CRC64` 算法得到的 `headerChecksum` 数量与使用 `MD5` 相同，与 `CRC32` 相差不大。
3. 考虑到降低读取数据的数量及计算量，`fdg` 使用 `CRC64` 算法；建议 `headerSize` 为 `40KB`。
